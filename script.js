...
function renderQuizHome() {
  ...
  const homeStartUpString = `<h2 id="intro">How well do you know the Bible?</h2>
...
function generateAnswerString(answers) {
  ...
  const answerHTML = answers.map((ans, index) =>
    generateAnswerElement(ans, index)
  );
...
function renderQuestion() {
  ...
  const questionHTMLString = `<fieldset id="questionAnswerSet">
            <legend>${currentQuestion.question}</legend>
            ${generateAnswerString(currentQuestion.answers)}
            <button id="js-submitAnswer">Show Answer</button>
        </fieldset>`;
...
function generateQuestionResultString(isCorrect) {
  return `<div id="result" role="none">
                <h2>${
                  isCorrect
                    ? "Hallelujah!"
                    : "Looks like you need more studying!"
                }</h2>
                ${
                  isCorrect
                    ? ""
                ...
                <p>You can read this story in ${
                  QUESTIONS[currentQuesNum].bibleText
                }.</p>
            </div>
